...
const MetricCard = ({ icon: Icon, color }: { icon: React.ElementType, color: string}) => {
    return (
        <div className="bg-card/50 border border-border rounded-xl p-3 transition-all duration-300 hover:border-primary/50 hover:-translate-y-1 flex items-center">
            <div className={`p-2 rounded-lg ${color}/20 text-${color} mr-2 shadow-lg flex-shrink-0`}>
                <Icon className={`w-4 h-4 ${color}`} />
            </div>
            <div className="w-full space-y-1.5">
              <Placeholder className="w-3/4 h-2" />
...
const AppCard = ({ title, icon: Icon, color }: { title: string, icon: React.ElementType, color: string}) => (
    <div className="group bg-card/50 rounded-xl border border-border hover:border-primary/50 transition-all duration-200 flex flex-col h-full hover:-translate-y-1 p-3">
        ...
        <div className="space-y-1 mb-2 flex-grow">
            <Placeholder className="w-full h-1.5" />
            <Placeholder className="w-5/6 h-1.5" />
        </div>
        <div className="w-full bg-secondary py-1 px-2 rounded-md flex items-center justify-center text-xs font-medium">
            <Placeholder className="w-1/3 h-2" />
        </div>
    </div>
)
...
export function DashboardMockup() {
    return (
        <div className="w-full h-auto min-h-[350px] rounded-xl bg-card flex flex-col overflow-hidden border-2 border-border shadow-2xl text-[10px]">
            {/* Header */}
            ...
            <main className="flex-1 p-4 overflow-hidden">
                <div className="mb-4 space-y-2">
                    <Placeholder className="w-1/3 h-4" />
                    <Placeholder className="w-1/2 h-3" />
                </div>
                
                {/* Metrics Section */}
                <div className="mb-6">
                    <Placeholder className="w-1/4 h-3 mb-3" />
