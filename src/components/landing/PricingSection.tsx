...
const plans: Plan[] = [
  ...
  },
  {
    ...
    signupPath: '#contato', // Enterprise plan should link to contact
  },
];
...
const PricingCard = ({ plan, isLegacy }: { plan: Plan, isLegacy?: boolean }) => {
    const CardContent = (
        <div className={`relative bg-white rounded-3xl p-8 shadow-neumorphism hover:shadow-neumorphism-hover transition-all duration-300 ${plan.isScaled ? 'ring-2 ring-primary transform lg:scale-105' : ''}`}>
          ...
          <div className={`w-full py-4 rounded-2xl font-medium transition-all duration-300 shadow-neumorphism hover:shadow-neumorphism-hover flex items-center justify-center ${plan.buttonClass}`}>
            {plan.buttonText}
          </div>
          {plan.footerText && (
    ...
    if (plan.signupPath.startsWith('#')) {
        return <a href={plan.signupPath}>{CardContent}</a>
    }

    return (
        <Link href={plan.signupPath}>
            {CardContent}
        </Link>
    );
}
